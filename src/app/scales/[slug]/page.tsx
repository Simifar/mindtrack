...
export default async function ScalePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <article className="container section detail-page"><Link className="back-link" href="/scales">← В каталог тестов</Link><div className="detail-header"><div><span className="tag">{item.tags[0]}</span><h1>{item.title}</h1><p className="detail-subtitle">{item.shortTitle}</p></div><FavoriteButton id={item.id} /></div><p className="lead narrow">{item.summary}</p><div className="detail-layout"><div><section className="prose-block"><h2>О тесте</h2><p>{item.description}</p><p><strong>Для кого:</strong> {item.audience}</p><p><strong>Время:</strong> около {item.durationMinutes} минут</p><Link className="button button-primary result-cta" href={`/results?test=${item.id}`}>Сохранить результат</Link></section><section className="notice"><strong>Важно</strong><p>Результат скрининга не является диагнозом. Если состояние беспокоит вас или мешает повседневной жизни, обратитесь к квалифицированному специалисту.</p></section></div><aside className="related-box"><h2>Связанные разделы</h2>{relatedTopics.map((topic) => <Link key={topic.id} href={`/topics/${topic.slug}`}>{topic.title}</Link>)}<div className="status-line"><Star size={16} /> {item.reviewStatus === "assessed" ? "Материал проверен редактором" : "Материал ожидает редакционной проверки"}</div></aside></div></article>;
}
