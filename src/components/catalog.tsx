  ...
  return (
    <div>
      <div className="filters" role="search">
        <label className="search-field"><Search size={18} aria-hidden="true" /><span className="sr-only">Поиск</span><input value={query} onChange={(event) => updateParams("q", event.target.value)} placeholder="Поиск по каталогу" /></label>
        <label><span className="sr-only">Раздел</span><select value={topic} onChange={(event) => updateParams("topic", event.target.value)}><option value="all">Все разделы</option>{topics.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label>
      </div>
      <p className="muted result-line">Найдено: {filtered.length}</p>
      ...
      {pageCount > 1 && <nav className="pagination" aria-label="Страницы каталога">{Array.from({ length: pageCount }, (_, index) => index + 1).map((item) => <Link className={item === currentPage ? "current" : ""} href={pageHref(item)} key={item}>{item}</Link>)}</nav>}
